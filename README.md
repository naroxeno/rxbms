...
`rust` 实现的现代 BMS 播放器：beatoraja 风格皮肤系统、低延迟音频、BGA 视频、完整游玩闭环
（选曲 → 游玩 → 结算）。
...
- **皮肤**：beatoraja 风格 Lua 皮肤（`assets/test_skin/`，Play5/Play7 按模式加载）——音符/判定/打击特效/血条/HUD 全部由皮肤驱动；BGA 作为皮肤 destination 渲染
  - 皮肤素材来源：B 站用户 [FAm_Renderer（space.bilibili.com/2585574）](https://space.bilibili.com/2585574/dynamic) 的游戏作品，仅供本地测试与开发使用，感谢作者
